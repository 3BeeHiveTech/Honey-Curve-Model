...
def validateaxis(axis) -> None:
    ...
    if axis_type == tuple:
        raise TypeError(
            "Tuples are not accepted for the 'axis' parameter. "
    ...
    if current_shape is None:
        ...
        if not negative_indexes:
            ...
            if new_size != current_size:
                raise ValueError(
                    "cannot reshape array of size {} into shape {}".format(current_size, new_shape)
                )
...
def check_reshape_kwargs(kwargs):
    ...
    if kwargs:  # Some unused kwargs remain
        raise TypeError(
            "reshape() got unexpected keywords arguments: {}".format(", ".join(kwargs.keys()))
        )
    return order, copy
