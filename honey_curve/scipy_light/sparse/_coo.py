...
from ._base import SparseEfficiencyWarning, _spbase, issparse, sparray
from ._data import _data_matrix, _minmax_mixin
from ._matrix import _array_doc_to_matrix, spmatrix
from ._sparsetools import coo_matvec, coo_tocsr, coo_todense
...
class _coo_base(_data_matrix, _minmax_mixin):
        ...
        if isinstance(arg1, tuple):
            if isshape(arg1):
                ...
                idx_dtype = self._get_index_dtype(
                    (row, col), maxval=max(self.shape), check_contents=True
                )
            ...
            if issparse(arg1):
                ...
                if shape is not None:
                    if check_shape(shape) != self._shape:
                        raise ValueError("inconsistent shapes: %s != %s" % (shape, self._shape))
    ...
    def todia(self, copy=False):
        ...
        if len(diags) > 100:
            ...
            warn(
                "Constructing a DIA matrix with %d diagonals " "is inefficient" % len(diags),
    ...
    def diagonal(self, k=0):
        ...
        if self.has_canonical_format:
            ...
            row, _, data = self._sum_duplicates(
                self.row[diag_mask], self.col[diag_mask], self.data[diag_mask]
            )
    ...
    def _with_data(self, data, copy=True):
        ...
        if copy:
            return self.__class__(
                (data, (self.row.copy(), self.col.copy())), shape=self.shape, dtype=data.dtype
            )
        else:
    ...
    def _mul_multivector(self, other):
        result = np.zeros(
            (other.shape[1], self.shape[0]), dtype=upcast_char(self.dtype.char, other.dtype.char)
        )
