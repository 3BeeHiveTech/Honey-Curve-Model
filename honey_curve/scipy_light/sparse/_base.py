...
class _spbase:
    ...
    def _getnnz(self, axis=None):
        ...
        """
        raise NotImplementedError("getnnz not implemented for %s." % self.__class__.__name__)
    ...
    def __repr__(self):
        ...
        return (
            f"<%dx%d sparse {sparse_cls} of type '%s'\n"
    ...
    def sum(self, axis=None, dtype=None, out=None):
        ...
        if axis is None:
            ...
            return (self @ self._ascontainer(np.ones((n, 1), dtype=res_dtype))).sum(
                dtype=dtype, out=out
            )
    ...
    def mean(self, axis=None, dtype=None, out=None):
        ...
        # axis = 0 or 1 now
        if axis == 0:
            return (inter_self * (1.0 / self.shape[0])).sum(axis=0, dtype=res_dtype, out=out)
        else:
            return (inter_self * (1.0 / self.shape[1])).sum(axis=1, dtype=res_dtype, out=out)
...
""",
    ...
    def getformat(self):
        ...
        """
        msg = (
            "`getformat` is deprecated and will be removed in v1.13.0; " "use `X.format` instead."
        )
        warn(msg, DeprecationWarning, stacklevel=2)
        return self.format
    ...
    def getnnz(self, axis=None):
        ...
        count_nonzero : Number of non-zero entries
        """
        msg = "`getnnz` is deprecated and will be removed in v1.13.0; " "use `X.nnz` instead."
        warn(msg, DeprecationWarning, stacklevel=2)
    ...
    def getcol(self, j):
        ...
        """
        msg = (
            "`getcol` is deprecated and will be removed in v1.13.0; " f"use `X[:, [{j}]]` instead."
        )
        warn(msg, DeprecationWarning, stacklevel=2)
