...
class _cs_matrix(_data_matrix, _minmax_mixin, IndexMixin):
    ...
    def __init__(self, arg1, shape=None, dtype=None, copy=False):
        ...
        if issparse(arg1):
            ...
            if isshape(arg1):
                ...
                if len(arg1) == 2:
                    # (data, ij) format
                    other = self.__class__(self._coo_container(arg1, shape=shape, dtype=dtype))
    ...
    def check_format(self, full_check=True):
        ...
        if self.indptr.dtype.kind != "i":
            warn(
                "indptr array has non-integer dtype ({})" "".format(self.indptr.dtype.name),
                stacklevel=3,
            )
        if self.indices.dtype.kind != "i":
            warn(
                "indices array has non-integer dtype ({})" "".format(self.indices.dtype.name),
                stacklevel=3,
            )
        ...
        if len(self.indptr) != major_dim + 1:
            raise ValueError(
                "index pointer size ({}) should be ({})" "".format(len(self.indptr), major_dim + 1)
            )
        if self.indptr[0] != 0:
        ...
        if issparse(other):
            ...
            # Row vector times matrix. other is a row.
                ...
                other = self._dia_container(
                    (other.toarray().ravel(), [0]), shape=(other.shape[1], other.shape[1])
                )
            ...
            # Column vector times matrix. other is a column.
                ...
                other = self._dia_container(
                    (other.toarray().ravel(), [0]), shape=(other.shape[0], other.shape[0])
                )
    ...
    def _maximum_minimum(self, other, npop, op_name, dense_check):
        if isscalarlike(other):
            if dense_check(other):
                ...
                mat = self.__class__(
                    (new_data, self.indices, self.indptr), dtype=new_data.dtype, shape=self.shape
                )
                return mat
        elif isdense(other):
    ...
    def _get_intXint(self, row, col):
        ...
        indptr, indices, data = get_csr_submatrix(
            M, N, self.indptr, self.indices, self.data, major, major + 1, minor, minor + 1
        )
    ...
    def _major_index_fancy(self, idx):
        ...
        csr_row_index(M, indices, self.indptr, self.indices, self.data, res_indices, res_data)
    ...
    def _major_slice(self, idx, copy=False):
        ...
        if step == 1:
            ...
            res_data = np.empty(nnz, dtype=self.dtype)
            csr_row_slice(
                start, stop, step, self.indptr, self.indices, self.data, res_indices, res_data
            )
    ...
    def _minor_index_fancy(self, idx):
        ...
        csr_column_index1(k, idx, M, N, self.indptr, self.indices, col_offsets, res_indptr)
    ...
    def resize(self, *shape):
        ...
        if hasattr(self, "blocksize"):
            ...
            if rm or rn:
                raise ValueError(
                    "shape must be divisible into {} blocks. "
