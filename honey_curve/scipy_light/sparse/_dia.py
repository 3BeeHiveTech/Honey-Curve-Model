...
from ._base import _formats, _spbase, issparse, sparray
from ._data import _data_matrix
from ._matrix import _array_doc_to_matrix, spmatrix
from ._sparsetools import dia_matvec
from ._sputils import check_shape, get_sum_dtype, getdtype, isshape, upcast_char, validateaxis
...
class _dia_base(_data_matrix):
            ...
            if isshape(arg1):
                ...
                try:
                    ...
                    self.offsets = np.atleast_1d(
                        np.array(
                            arg1[1], dtype=self._get_index_dtype(maxval=max(shape)), copy=copy
                        )
                    )
    ...
    def __repr__(self):
        ...
        return (
            ...
            % (self.shape + (self.dtype.type, self.nnz, self.data.shape[0], format))
        )
    ...
    def _getnnz(self, axis=None):
        if axis is not None:
            raise NotImplementedError("_getnnz over an axis is not implemented " "for DIA format")
        M, N = self.shape
        nnz = 0
    ...
    def transpose(self, axes=None, copy=False):
        ...
        data = np.hstack(
            (self.data, np.zeros((self.data.shape[0], pad_amount), dtype=self.data.dtype))
        )
        data = data[r, c]
        return self._dia_container((data, offsets), shape=(num_cols, num_rows), copy=copy)
    ...
    def tocsc(self, copy=False):
        ...
        return self._csc_container((data, indices, indptr), shape=self.shape, dtype=self.dtype)
    ...
    def resize(self, *shape):
        ...
        if M > self.shape[0] and np.any(self.offsets + self.shape[0] < self.data.shape[1]):
            # explicitly clear values that were previously hidden
            mask = self.offsets[:, None] + self.shape[0] <= np.arange(self.data.shape[1])
            self.data[mask] = 0
